---
title: "Into the tidyverse, Session 3"
...
::: callout-note
Some of this is review from session 1, but now we'll talk about R-specific applications!
:::
...
Before we can use these libraries, we must install them. Let's install a few libraries that are particularly important. You should type this into your console, rather than your script.
...
::: callout-warning
Note that if you've installed these libraries in the past, you don't need to do it again! Call `install.packages` when you want to install a package for the first time, or when you want to update a package to the latest version. We'll later learn how to load installed packages.
:::
...
![](./images/csv_plaintext.png){fig-alt="On the left is a CSV open in Microsoft Excel. It looks like a regular table of data. On the right is the same CSV open in a plaintext editor. Each row is clearly a row of the data. The columnar structure is harder to see, however, since the values in each column are separated only by commas."}
...
First things first, we'll load the libraries we need for today's tutorial. Note that we're using the tidyverse libraries `readr` and `tibble`. The `here` library is used for convenient pathing, and you'll note that it gives you a message about where it will start pathing.
...
::: callout-note
The `knitr::kable` function is just something I'm using to print pretty tables when converting the tutorial code to HTML. You don't need to use it in your own code!
:::
...
```{r readr-specify-datatypes}
tidy_read_mod <- here("data", "PVD_2020_Property_Tax_Roll.csv") %>%
  read_csv(
    col_types = cols(
      ZIP_POSTAL = col_character(),
      plat = col_character()
    )
  )
```

One last note here, about coding style. Let's say I asked you to read a novel you enjoy, but after I've stripped out all of the paragraphs, indentation, and a good amount of the punctuation. I rather suspect that your reading experience would be terrible. The same goes for code. Much of the time, we intend for our code to be read by other people, and so we should aspire to write pleasantly-styled code.

The following code *works*, but is written in a way that makes it difficult to read.

```{r messy-code}
#| eval: false

here("data","PVD_2020_Property_Tax_Roll.csv") %>% read_csv(col_types=cols(ZIP_POSTAL=col_character()),col_select=c(TAX_MAP,SHORT_DESC,FORMATED_ADDRESS,ZIP_POSTAL))
```

The same code becomes much more readable when styled.
...
here("data","PVD_2020_Property_Tax_Roll.csv") %>%
  read_csv(
    # Make sure ZIP code isn't accidentally read as numeric
    col_types = cols(ZIP_POSTAL = col_character()),
    # Don't need all the columns, so select just the ones we want
    col_select = c(TAX_MAP, SHORT_DESC, FORMATED_ADDRESS, ZIP_POSTAL)
  )
```

In college, I took an intro to programming class where about a third of each assignment grade was determined by the cleanliness of our coding style. My friends who are industry software developers (think Google, Facebook) are required to adhere to strict style guidelines, or else their code gets a demerit. Yes, it's that important!

Here are some general tips for you to consider:

1.  In your script window, you'll see a vertical line on your screen. That vertical line indicates when you've typed 80 characters on the same line. If your code is longer than 80 characters, break it up into several lines.
2.  When you're calling a function that takes several arguments, consider defining each argument on its own line. You can see an example of this directly above.
3.  Use parentheses and indentation to your advantage. In the example above, you can see how `read_csv` spans multiple lines. Especially in long pipes, you'll want to visually distinguish each function as much as you can. Here, you can see how the parenthesis placement delineates where the function starts and ends.
4.  Pipes go at the end of each line, and there should always be a new line after a pipe.
5.  You can use comments to make notes along the way, and in a long pipe, strategic use of comments can also help delineate different tasks you want to accomplish (which might require several functions, not just one).
6.  Ultimately, *readability is key*. Do whatever you must to maintain readability.
...
Verify that `tidy_read_mod` correctly read the columns `plat` and `ZIP_POSTAL` as strings.

## Exercise 2

The Johns Hopkins covid-19 tracker maintains the latest count of confirmed coronavirus cases in the U.S. at the following URL:

`https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_US.csv`

*Without downloading the file to your hard drive*, read it into R, and assign it to a variable named `covid_usa`.

Then, save `covid_usa` as a CSV in the `output` folder.

## Exercise 3

In the `data` folder, there is a dataset listing all of the [bank locations in Louisville, KY](https://catalog.data.gov/dataset/bank-branches-data/resource/769ef747-f5b8-4154-846e-2f347d45a7ce). The file name is `BankBranchesData.txt`. Read this dataset into R and assign it to a variable named `bank_branches`.

You may be tempted to manually the file's extension from `.txt` to something else. Resist that temptation. We always want to avoid making any manual changes to raw data; any such changes should be implemented using code, in a manner that preserves the raw data as-is.
