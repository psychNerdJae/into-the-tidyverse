...
Before we can use these libraries, we must install them. Let's install a few libraries that are particularly important. Note that if you've installed these libraries in the past, you don't need to do it again! You should type this into your console, rather than your script.
...
```{r csv-img}
#| echo = FALSE
knitr::include_graphics(here::here("code", "images", "csv_plaintext.png"))
```
...
```{r messy-code}
#| eval = FALSE
here("data","PVD_2020_Property_Tax_Roll.csv") %>% read_csv(col_types=cols(ZIP_POSTAL=col_character()),col_select=c(TAX_MAP,SHORT_DESC,FORMATTED_ADDRESS,ZIP_POSTAL))
```
...
Exercise 1: Verify that `tidy_read_mod` correctly read the columns `plat` and `ZIP_POSTAL` as strings.

Exercise 2: The Johns Hopkins covid-19 tracker maintains the latest count of confirmed coronavirus cases in the U.S. at the following URL:
